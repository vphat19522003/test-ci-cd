...
import { useLogOut, useResendOTP, useVerifyOTP } from '@app/api/hooks/auth.hook';
import ButtonForm from '@app/components/atoms/button';
import { OTPCountDownSeconds } from '@app/constants/date';
import { digitRegex } from '@app/constants/regex';
import { clearUser, setUser } from '@app/redux/authSlice';
import { paths } from '@app/routes/paths';
import { RootState } from '@app/store';
import { IErrorResponse } from '@app/types/common';

import { AccountVerifySchema, AccountVerifyValidateType } from './schema';
...
const AccountVerify = (): JSX.Element => {
  ...
  const {
    ...
    handleSubmit
  } = useForm<AccountVerifyValidateType>({
    resolver: zodResolver(AccountVerifySchema),
    defaultValues: { code: ' '.repeat(6), email: user?.email }
  });
  ...
  const handleSubmitForm = (value: AccountVerifyValidateType) => {
    verifyOTP(value, {
      onSuccess: (data) => {
  ...
  const handleClickResendOTP = () => {
    ...
    resendOTP(
      {},
      {
        onSuccess: (data) => {
          console.log(data);
          localStorage.setItem('lastOTPSent', currentTime.toString());
          toast.success(data.message);
        },
        onError: (err: IErrorResponse) => {
          toast.error(err.response.data.message as string);
  ...
  useEffect(() => {
    const lastOTPSent = localStorage.getItem('lastOTPSent');
    const currentTime = new Date().getTime();
    const timeDiff = (currentTime - Number(lastOTPSent)) / 1000;

    if (!lastOTPSent || timeDiff > OTPCountDownSeconds) {
      resetTimer();
      resendOTP(
        {},
        {
      ...
      toast.info('OTP is already send. Please check your email');
    }
  }, []);
